...
class ConfigTestingServer:
    ...
    @routes.post("/turnoff_config")
    async def turnoff_config(request: web.Request) -> web.Response:  # type: ignore
        inspect_header(request)
        if client_not_authorized(request):
            return show_not_authorized(request.app)
...
async def show_syssettings_page(main_app, popup_msg="") -> web.Response:
    ...
    tbl += (
        indent(7)
        + f'<tr><td style="width: 230px;"><label for="{id_name}">{prompt}</label></td><td style="width: 55px;"></td><td></td>'
    ...
    tbl += indent(6) + "</form>"
    if rtr.is_config_mode():
        ## Set "Konfig" to "Anwesend"
        id_name = "turnoff_config"
        prompt = "Konfig Modus ausschalten"
        tbl += indent(6) + '<form action="test/turnoff_config" method="post">\n'
        tbl += (
            indent(7)
            + f'<tr><td><label for="{id_name}">{prompt}</label></td><td></td><td></td>'
            + f'<td><input name="btn_{id_name}" type="submit" id="btn_{id_name}" value="Ausschalten"/></td></tr>\n'
        )
        tbl += indent(6) + "</form>"
